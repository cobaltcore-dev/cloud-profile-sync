...
build/cloud-profile-sync: FORCE generate
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/cloud-profile-sync .
...
install: FORCE build/cloud-profile-sync
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/cloud-profile-sync "$(DESTDIR)$(PREFIX)/bin/cloud-profile-sync"
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/cobaltcore-dev/cloud-profile-sync\e[0m\n"
	@goimports -w -local github.com/cobaltcore-dev/cloud-profile-sync $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/cloud-profile-sync\e[0m     Build cloud-profile-sync.\n"
