...
func (f *fakeFactory) Create(params ocirepo.Params, _ int64, _ logr.Logger, _ map[string]string, _ *v1alpha1.ImageFilter) (ossync.Source, error) {
	return &fakeOCISource{}, nil
}

type emptyFactory struct{}

func (f *emptyFactory) Create(params ocirepo.Params, parallel int64, _ logr.Logger, _ map[string]string, _ *v1alpha1.ImageFilter) (ossync.Source, error) {
...
var _ = Describe("The ManagedCloudProfile reconciler", func() {
	...
	It("keeps existing images when updates are paused", func(ctx SpecContext) {
	...
	It("does not overwrite existing images or provider config once paused", func(ctx SpecContext) {
		var mcp v1alpha1.ManagedCloudProfile
		...
		// Pause and remove the update source; a paused reconcile must not wipe or
		// change the previously synced machine images or provider config.
		Eventually(func(g Gomega) {
